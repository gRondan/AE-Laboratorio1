...
skeleton newGA
{
	...
	Problem::Problem ():_dimension(0),_sueldo_empleado(NULL),_habilidad_empleado(NULL),_disponibilidad_empleado(NULL),_tiempo_tarea(NULL),_limite_proyecto(0)
	{}
	...
	istream& operator>> (istream& is, Problem& pbm)
	{
		...
		string str3(line);
		pbm._sueldo_empleado= new int[pbm._dimension];
		for(int i=0; i < pbm._dimension; i++){
				string aux = pbm.getfield(str3, i + 1);
				int aux_i = stoi(aux);
				pbm._sueldo_empleado[i] = auxi;
		}
		pbm._habilidad_empleado= new int[pbm._dimension];
		for(int i=0; i < pbm._dimension; i++){
				string aux = pbm.getfield(str3, i + 1);
				int aux_i = stoi(aux);
				pbm._habilidad_empleado[i] = auxi;
		}
		pbm._sueldo_empleado= new int[pbm._dimension];
		for(int i=0; i < pbm._dimension; i++){
				string aux = pbm.getfield(str3, i + 1);
		...
		pbm._disponibilidad_empleado= new int[pbm._dimension];
		for(int i=0; i < pbm._dimension; i++){
				string aux = pbm.getfield(str3, i + 1);
				int aux_i = stoi(aux);
				pbm._disponibilidad_empleado[i] = auxi;
		}
		pbm._tiempo_tarea= new int[pbm._dimension];
		for(int i=0; i < pbm._dimension; i++){
				string aux = pbm.getfield(str3, i + 1);
				int aux_i = stoi(aux);
				pbm._tiempo_tarea[i] = auxi;
		}
		string _limite_proyecto = pbm.getfield(str, 1);
		int _limite_proyecto_i = stoi(_limite_proyecto);
		pbm._cant_contenedores = _limite_proyecto_i;
		return is;
	}
